        ...
        for j in range(order + 1):
            for k in range(order + 1):
                for l in range(order + 1):
                    ...
                    Cheb_inputs.append(np.cos(np.pi * np.array(inputs[-1]) / order))
                    real_inputs.append((Cheb_inputs[-1] + 1) / 2 * (intervals[:, 1] - intervals[:, 0]) + intervals[:, 0])
                    data_vec.append(expensive_func(real_inputs[-1][0], real_inputs[-1][1], real_inputs[-1][2], real_inputs[-1][3]))

    return inputs, Cheb_inputs, real_inputs, data_vec


inputs, Cheb_inputs, real_inputs, data_vec = make_data()

# Create the interpolation object
interp = ClassicCheb(Cheb_inputs, data_vec, intervals, order, input_type='Cheb')
...
if np.all(interp.get_derivative(point, axis=(0, 3, 2)) == Cheb_derivative[[0, 3, 2]]):
    print('passed axis test 2')
else:
    raise Exception('Failed axis test 2')

if np.all(interp.get_derivative(point, axis=(0, 1, 2, 3)) == Cheb_derivative):
...
plt.title('Convergence plot of ClassicCheb derivative')
...
plt.legend()
plt.savefig('ClassicCheb_test_function_derivative_convergence.pdf')
plt.show()
